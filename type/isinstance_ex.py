...
data_list = [1, 2, 3]
print(f"\n({data_list}) 데이터가 {type(data_list)}일 때 isinstance str로 설정 결과: {isinstance(data_list, list)}")
print(f"({data_list}) 데이터가 {type(data_list)}일 때 isinstance int로 설정 결과: {isinstance(data_list, str)}")